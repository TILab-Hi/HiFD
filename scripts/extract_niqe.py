"""No-reference NIQE quality score per image (pyiqa), written to LMDB.

Per-key value (pickled):
    {"niqe": float}

Input: 1+ per-frame LMDB(s). Output: single LMDB keyed by the same keys.
"""
...
def run(args):
    ...
    for key_str, rec in iter_lmdb(lmdb_paths):
        ...
        t = decode_to_tensor(rec['image'])
        if t is None:
            n_skip_run += 1
        else:
            shp = tuple(t.shape)
            # Flush on shape change or batch full
